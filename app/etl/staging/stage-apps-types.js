...
const stageAppsTypes = async () => {
  ...
  const file = await storage.downloadFile(`${storageConfig.appsTypes.folder}/export.csv`, tempFilePath)
  console.log(file)
  const columns = [
    'CHANGE_TYPE',
    'CHANGE_TIME',
  ...
  return runEtlProcess({ tempFilePath, columns, table: appsTypesTable, mapping })
}
