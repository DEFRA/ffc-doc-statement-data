...
const { calculationsDetailsDelinked } = require('../../constants/etl-tables')
...
const stageCalculationDetails = async () => {
  ...
  const mapping = [
    ...
    { column: sourceColumnNames.CALCULATION_DT, targetColumn: targetColumnNames.calculationDt, targetType: DATE, format: monthDayYearDateTimeFormat },
    { column: sourceColumnNames.RANKED, targetColumn: targetColumnNames.ranked, targetType: NUMBER }
  ]

  return downloadAndProcessFile('calculationsDetailsDelinked', calculationsDetailsDelinked, columns, mapping)
}
