...
WITH new_data AS (
  SELECT
    transdate,
    invoiceid,
    0 AS scheme,  -- Set scheme as 0
    fund,
    0 AS marketingyear,  -- Set marketingyear as 0
    "month",
    quarter,
    CAST(
      COALESCE(
    ...
    change_type,
    recid,
    0 AS siti_invoice_id,  -- Set siti_invoice_id as 0
    0 AS claim_id  -- Set claim_id as 0
  FROM etl_stage_finance_dax D
  WHERE LENGTH(accountnum) = 10
    AND etl_id BETWEEN :idFrom AND :idTo
),
updated_rows AS (
  ...
  SET
    ...
    invoiceid = new_data.invoiceid,
    etl_inserted_dt = NOW(),
    scheme = new_data.scheme,  -- Set scheme as 0
    marketingyear = new_data.marketingyear,  -- Set marketingyear as 0
    siti_invoice_id = new_data.siti_invoice_id,  -- Set siti_invoice_id as 0
    claim_id = new_data.claim_id  -- Set claim_id as 0
  FROM new_data
  WHERE new_data.change_type = 'UPDATE'
    AND interm.recid = new_data.recid
  RETURNING interm.recid
...
INSERT INTO etl_interm_finance_dax (
  transdate,
  invoiceid,
  scheme,  -- Set scheme as 0
  fund,
  marketingyear,  -- Set marketingyear as 0
  "month",
  quarter,
  TRANSACTION_AMOUNT,
  agreementreference,
  PAYMENT_REF,
  recid,
  siti_invoice_id,  -- Set siti_invoice_id as 0
  claim_id  -- Set claim_id as 0
)
SELECT
  transdate,
  invoiceid,
...
const loadIntermFinanceDAX = async (startDate, transaction, folder = storageConfig.financeDAX.folder, query = defaultQuery) => {
...
const loadIntermFinanceDAXDelinked = async (startDate, transaction) => {
  return loadIntermFinanceDAX(startDate, transaction, storageConfig.financeDAXDelinked.folder, delinkedQuery)
}
