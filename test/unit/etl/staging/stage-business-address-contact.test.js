...
describe('stageBusinessAddressContacts', () => {
  ...
  test('should include nonProdTransformer when fakeData is true', async () => {
    ...
    await stageBusinessAddressContacts()
    expect(runEtlProcess).toHaveBeenCalledWith(expect.objectContaining({
      nonProdTransformer: expect.any(Array)
    }))
  })

  test('should include excludedFields when excludeCalculationData is true', async () => {
    jest.resetModules()
    jest.doMock('../../../../app/config', () => ({
      etlConfig: {
        excludeCalculationData: true,
    ...
    await stageBusinessAddressContacts()
    expect(runEtlProcess).toHaveBeenCalledWith(expect.objectContaining({
      excludedFields: expect.any(Array)
    }))
  })
})
